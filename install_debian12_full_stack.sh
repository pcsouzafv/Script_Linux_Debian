...
check_required_ports() {
    local required_ports=(80 443 3306 10050 10051)
    local busy_ports=()
    local port
    ...
    if [ ${#busy_ports[@]} -gt 0 ]; then
        error "As portas ${busy_ports[*]} já estão em uso. Revise containers Docker, proxies ou serviços locais antes de executar este instalador."
    fi
}
...
log "Validando portas obrigatórias do host..."
check_required_ports
